...
Vue.component('load-summary', {
	...
	methods: {		
		loadFileInDatabase: async function(){
			var url = APPURL+'upload/load-file-in-database';
			await axios.get(url);
			this.status = 2;
		},
		getStatusProcess: async function(){
			var url_status_proceess = APPURL+'process/get-status-process';
			await axios.get(url_status_proceess)		
			.then((res) => {
				if (res.data.success) {
